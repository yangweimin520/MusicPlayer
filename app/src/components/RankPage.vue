<template>
  <div id="rankpage">
    ...
    <div class="list" :style="{background:color}" v-if="topListData!=null">
      <ul>
        <li
          class="border-1px border-1px-after"
          v-for="(item, index) in topListData.songlist"
          :key="index"
        >
  ...
  methods: {
    ...
    showMenu: function(num) {
      ...
      this.$store.dispatch("notifyActionSheet", {
        menus: {
          "title.noop":
            this.topListData.songlist[num].data.songorig +
            '<br/><span style="color:#666;font-size:12px;">' +
            this.getSingerStr(this.topListData.songlist[num].data.singer) +
            "</span>",
          playAsNext: "下一首播放",
